...
public class FixedDepositImediateLiquidationApprovalFactory : IApprovalFactory
{
    private readonly ChevronCoopDbContext _dbContext;
    private readonly IMapper _mapper;
    private readonly IMediator _mediator;

    public FixedDepositImediateLiquidationApprovalFactory(ChevronCoopDbContext dbContext, IMapper mapper, IMediator mediator)
    {
    ...
    public async Task<CommandResult<bool>> Process(Approval request, string? approvedById, string? comment, ApprovalStatus status)
    {
        ...
        try
        {
            ...
            var model = _dbContext.FixedDepositLiquidations
                   .FirstOrDefault(x => x.Id == request.EntityId);

            
            if (model != null)
            {
                var payload = System.Text.Json.JsonSerializer.Deserialize<CreateFixedDepositLiquidationCommand>(request.Payload);
