...
public class SpecialIncreaseDecreaseApprovalFactory : IApprovalFactory
{
    private readonly ChevronCoopDbContext _dbContext;
    private readonly IMapper _mapper;
    private readonly IMediator _mediator;

    public SpecialIncreaseDecreaseApprovalFactory(ChevronCoopDbContext dbContext, IMapper mapper, IMediator mediator)
    {
    ...
    public async Task<CommandResult<bool>> Process(Approval request, string? approvedById, string? comment, ApprovalStatus status)
    {
        ...
        try
        {
            ...
            var model = _dbContext.SpecialDepositIncreaseDecreases.FirstOrDefault(x => x.Id == request.EntityId);

            if (model != null)
            {
                var command = new UpdateSpecialDepositIncreaseDecreaseCommand
                {
                    Id = request.EntityId,
                    SpecialDepositAccountId = model.SpecialDepositAccountId,
                    Amount = model.Amount,
                    ContributionChangeRequest = model.ContributionChangeRequest
                };
