...
public class SavingIncreaseDecreaseApprovalFactory : IApprovalFactory
{
    private readonly ChevronCoopDbContext _dbContext;
    private readonly IMapper _mapper;
    private readonly IMediator _mediator;

    public SavingIncreaseDecreaseApprovalFactory(ChevronCoopDbContext dbContext, IMapper mapper, IMediator mediator)
    {
    ...
    public async Task<CommandResult<bool>> Process(Approval request, string? approvedById, string? comment, ApprovalStatus status)
    {
        ...
        try
        {
            ...
            var model = _dbContext.SavingsIncreaseDecreases.FirstOrDefault(x => x.Id == request.EntityId);

            if (model != null)
            {
                var command = new UpdateSavingsIncreaseDecreaseCommand
                {
                    Id = request.EntityId,
