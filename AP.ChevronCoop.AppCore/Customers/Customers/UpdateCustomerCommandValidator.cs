...
public partial class UpdateCustomerCommandValidator : AbstractValidator<UpdateCustomerCommand>
{
    ...
    private readonly ILogger<UpdateCustomerCommandValidator> logger;
    public UpdateCustomerCommandValidator(ChevronCoopDbContext appDbContext, ILogger<UpdateCustomerCommandValidator> _logger)
    {
        ...
        RuleFor(p => p).Custom((data, context) =>
        {
            ...
            var checkId = dbContext.Customers.Where(r => r.Id == data.Id).Any();
            if (!checkId)
            {
                context.AddFailure(
