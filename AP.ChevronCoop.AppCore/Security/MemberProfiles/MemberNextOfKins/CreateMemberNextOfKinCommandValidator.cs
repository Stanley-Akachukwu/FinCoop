...
public class CreateMemberNextOfKinCommandValidator : AbstractValidator<CreateMemberNextOfKinCommand>
{

    private readonly ChevronCoopDbContext dbContext;
    private readonly ILogger<CreateMemberNextOfKinCommandValidator> logger;
    public CreateMemberNextOfKinCommandValidator(ChevronCoopDbContext appDbContext, ILogger<CreateMemberNextOfKinCommandValidator> _logger)
    {
        ...
        RuleFor(p => p).Custom((data, context) =>
        {
            ...
            var NOKCount = dbContext.MemberNextOfKins.Count(x => x.ProfileId == data.ProfileId);
            if (NOKCount >= 5)
            {
                context.AddFailure(
              new ValidationFailure(nameof(data.ProfileId),
