...
public class CreatePayrollDeductionItemCommandValidator : AbstractValidator<CreatePayrollDeductionItemCommand>
{
    private readonly ChevronCoopDbContext dbContext;
    private readonly ILogger<CreatePayrollDeductionItemCommandValidator> logger;

    public CreatePayrollDeductionItemCommandValidator(ChevronCoopDbContext appDbContext,
      ILogger<CreatePayrollDeductionItemCommandValidator> _logger)
    {
        ...
        RuleFor(p => p).Custom((data, context) =>
        {
            /*
                  ...
                  var checkName = dbContext.PayrollDeductionItems.Where(r => r.Name.ToLower() == data.Name.ToLower() 
          && r.CodeTypeId == data.CodeTypeId).Any();
                  ...
                  var checkCode = dbContext.PayrollDeductionItems.Where(r => r.Code.ToLower() == data.Code.ToLower() 
          && r.CodeTypeId != data.CodeTypeId).Any();
                  if (checkCode)
                  {
                      context.AddFailure(
