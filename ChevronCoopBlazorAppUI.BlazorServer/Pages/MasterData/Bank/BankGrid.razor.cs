...
namespace ChevronCoop.Web.AppUI.BlazorServer.Pages.MasterData.Bank
{
    [Authorize]
    public partial class BankGrid
    {
        [Inject]
        ILogger<BankGrid> Logger { get; set; }
        ...
        SfGrid<BankMasterView> grid;

        string GRID_API_RESOURCE => $"{Config.ODATA_VIEWS_HOST}/{nameof(BankMasterView)}";
        ...
        CreateBankCommand CreateModel { get; set; }
        UpdateBankCommand UpdateModel { get; set; }
        DeleteBankCommand DeleteModel { get; set; }

        BankCreateForm createForm;
        Drawer createDrawer;
        bool showCreateDrawer { get; set; } = false;

        BankEditForm editForm;
        Drawer editDrawer;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            CreateModel = new CreateBankCommand();
            UpdateModel = new UpdateBankCommand();
            DeleteModel = new DeleteBankCommand();
        ...
        public void ActionCompletedHandler(ActionEventArgs<BankMasterView> args)
        {
        ...
        private async Task OnViewRow(BankMasterView row)
        {
        }

        private async Task OnEditRow(BankMasterView row)
        {
            UpdateModel = Mapper.Map<UpdateBankCommand>(row);

            await onEdit();
        }


        private async Task OnDeleteRow(BankMasterView row)
        {
            bool isOk = false;
            ...
            if (isOk)
            {
                ...
                var rsp = await DataService.Delete<DeleteBankCommand, CommandResult<string>>(nameof(Bank), DeleteModel);
        ...
        private async Task OnSearch()
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchText))
            {
                ...
                {
                    Field = nameof(BankMasterView.Code),
                    Operator = "contains",
                    value = searchText
                };
