...
{
    [Authorize]
    public partial class BankCreateForm
    {
        ...
        [Parameter]
        public CreateBankCommand Model { get; set; }


        [Parameter]
        public EventCallback<CreateBankCommand> ModelChanged { get; set; }
        ...
        [Inject]
        ILogger<BankCreateForm> Logger { get; set; }
        ...
        {
            var rsp = await DataService.Create<CreateBankCommand, CommandResult<BankViewModel>>(
                nameof(Bank), Model);
        ...
        public async Task OnCancel()
        {
            if (reloadGrid)
            {
                NavigationManager.NavigateTo("/setup/banks", true);
                reloadGrid = false;
            }
            ShowModal = false;
