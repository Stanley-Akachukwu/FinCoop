...
{
    public partial class DepartmentGrid
    {
        [Inject]
        ILogger<DepartmentGrid> Logger { get; set; }
        ...
        SfGrid<DepartmentMasterView> grid;

        string GRID_API_RESOURCE => $"{Config.ODATA_VIEWS_HOST}/{nameof(DepartmentMasterView)}";
        ...
        CreateDepartmentCommand CreateModel { get; set; }
        UpdateDepartmentCommand UpdateModel { get; set; }
        DeleteDepartmentCommand DeleteModel { get; set; }

        DepartmentCreateForm createForm;
        Drawer createDrawer;
        bool showCreateDrawer { get; set; } = false;

        DepartmentEditForm editForm;
        Drawer editDrawer;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            CreateModel = new CreateDepartmentCommand();

            UpdateModel = new UpdateDepartmentCommand();

            DeleteModel = new DeleteDepartmentCommand();

            QueryGrid = new Query();
        }


        public void ActionCompletedHandler(ActionEventArgs<DepartmentMasterView> args)
        {
        ...
        private async Task OnViewRow(DepartmentMasterView row)
        {
        }

        private async Task OnEditRow(DepartmentMasterView row)
        {
            UpdateModel = Mapper.Map<UpdateDepartmentCommand>(row);

            await onEdit();
        }


        private async Task OnDeleteRow(DepartmentMasterView row)
        {
            bool isOk = false;
            ...
            if (isOk)
            {
                ...
                var rsp = await DataService.Delete<DeleteDepartmentCommand, CommandResult<string>>(nameof(Department),
                    DeleteModel);
        ...
        private async Task OnSearch()
        {
            ...
            if (!string.IsNullOrWhiteSpace(searchText))
            {
                ...
                // {
                // 	Field = nameof(DepartmentMasterView.Code),
                // 	Operator = "contains",
                // 	value = searchText
                // };
