...
{
    public partial class CreateApprovalWorkflowComp
    {
        [Inject]
        ...
        public async Task AddGroupToList(GroupToApproveViewModel g)
        {
            ...
            if (selectedGroups.Count == 1 && selectedGroups.FirstOrDefault().GroupApprovalSequence == g.GroupApprovalSequence)
            {
                await notificationService.Open(new NotificationConfig()
                {
        ...
        private async Task OnGroupValueSelectedhandler(SelectEventArgs<ApprovalGroupMasterView> args)
        {
          ...
          //  approvalGroup.GroupApprovalSequence = group.
        ...
        public async Task OnCreateWorkFlow(CreateApprovalWorkflowCommand model)
        {
            ...
            var rsp = await DataService.CreateApprovalWorkflow<CreateApprovalWorkflowCommand, CommandResult<ApprovalWorkflowViewModel>>(model);
            ...
            {
                await _auditLogService.LogAudit("Workflow Creation.", $"Created {ApprovalWorkflowType} workflow-", "Security", payload, CurrentUser);
