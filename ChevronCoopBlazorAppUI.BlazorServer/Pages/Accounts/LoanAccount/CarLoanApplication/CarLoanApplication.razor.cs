...
namespace ChevronCoop.Web.AppUI.BlazorServer.Pages.Accounts.LoanAccount.CarLoanApplication
{
    public partial class CarLoanApplication
    {
        private FluentValidationValidator? _fluentValidationValidator_Info;
        private FluentValidationValidator? _fluentValidationValidator_CarDetail;
        ...
        [Inject]
        ILogger<CarLoanApplication> Logger { get; set; }
        ...
        public ApplianceDetails CarDetail { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            CarDetail = new ApplianceDetails();
        ...
        {
            if (await _fluentValidationValidator_CarDetail.ValidateAsync())
            {
        ...
        public async Task LoadDropDown()
        {
            combobox_loanproducts =
                $"{Config.API_HOST}/{nameof(LoanProduct)}/{nameof(LoanProductType.CAR_LOAN)}/userProducts/{ApplicationUserId}";
            combobox_preferredAccounts =
                $"{Config.ODATA_VIEWS_HOST}/{nameof(CustomerBankAccountMasterView)}?$filter=CustomerId eq '{CustomerMasterView.Id}' and IsDeleted eq false";
        ...
        private async Task ValueChangeHandler(ChangeEventArgs<string, LoanProductViewModel> args)
        {
            ...
            if (SelectedLoanProduct != null)
            {
                ...
                else if (CommonResponseDTO.IsError)
                {
                    ...
                    _navigationManager.NavigateTo($"/account/carloanapplication", true);
