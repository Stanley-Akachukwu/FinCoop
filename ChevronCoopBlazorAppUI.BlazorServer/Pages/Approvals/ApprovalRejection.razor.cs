...
{
    public partial class ApprovalRejection
    {
        [Inject]
        NavigationManager navigationManager { get; set; }

        [Inject]
        ILogger<ApprovalRejection> Logger { get; set; }
        ...
        public CreateFixedDepositAccountApplicationCommand CreateFixedDepositAccountApplicationCommand { get; set; }
        ...
        private async Task GetApprovalDetails()
        {
            try
            {
                ...
                if (ViewModel.ApprovalType.Equals(ApprovalType.FIXED_DEPOSIT_APPLICATION.ToString()))
                {
                    CreateFixedDepositAccountApplicationCommand = new CreateFixedDepositAccountApplicationCommand();
                    CreateFixedDepositAccountApplicationCommand =
                        System.Text.Json.JsonSerializer.Deserialize<CreateFixedDepositAccountApplicationCommand>(
                            ViewModel.Payload);
                }

                else if (ViewModel.ApprovalType.Equals(ApprovalType.SAVING_DEPOSIT_APPLICATION.ToString()))
                {
                    CreateSavingsAccountApplicationCommand = new CreateSavingsAccountApplicationCommand();
                    CreateSavingsAccountApplicationCommand =
                        System.Text.Json.JsonSerializer.Deserialize<CreateSavingsAccountApplicationCommand>(
                            ViewModel.Payload);
                }
                ...
                if (ViewModel.ApprovalType.Equals(ApprovalType.DEPOSIT_PRODUCT.ToString()))
                {
