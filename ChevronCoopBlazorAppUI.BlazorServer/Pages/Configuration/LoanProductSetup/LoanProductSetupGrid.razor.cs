...
{
    public partial class LoanProductSetupGrid
    {
        ...
        [Inject]
        ILogger<LoanProductSetupGrid> Logger { get; set; }
        ...
        //ApproveLoanProductCommand ApproveCommand { get; set; }


        SfGrid<LoanProductMasterView> grid;

        string GRID_API_RESOURCE => $"{Config.ODATA_VIEWS_HOST}/{nameof(LoanProductMasterView)}?$orderby=DateCreated desc";
        ...
        public PublishLoanProductCommand command { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                ...
                command = new PublishLoanProductCommand();
                ...
                await _auditLogService.LogAudit("Accessed Loan Product list", "Accessed Loan Product list", "Security",
        ...
        {
            _navigationManager.NavigateTo($"/LoanProductSetup/Manage/{filter}", forceLoad: true);
        }
        ...
        private async Task ExecuteFilteringAsync()
        {
            switch (filter)
            {
                case "approved":
                    ...
                    {
                        Field = nameof(LoanProductMasterView.Status),
                        Operator = "equal",
                        value = nameof(ProductStatus.APPROVED)
                    };
                    var activeFilter = new WhereFilter
                    {
                        Field = nameof(LoanProductMasterView.Status),
                        Operator = "equal",
                        value = nameof(ProductStatus.ACTIVE)
                    };
        ...
        private async Task onApprovingProductSetup(LoanProductMasterView row)
        {
            //ApproveCommand = new ApproveDepositProductCommand();
        ...
        public void ActionCompletedHandler(ActionEventArgs<LoanProductMasterView> args)
        {
            ...
            try
            {
                ...
                if (checkIsSuccessfull)
                {
                    ...
                    var rsp = await DataService
                        .ProcessRequest<PublishLoanProductCommand, CommandResult<LoanProductViewModel>>(
                            nameof(LoanProduct), "publish", command);
                    ...
                    else
                    {
                        ...
                        _navigationManager.NavigateTo($"/LoanProductSetup/Manage/all", forceLoad: true);
                    }
                }
            }
            catch (Exception ex)
        ...
        private async Task SearchFiltering()
        {
            if (!string.IsNullOrWhiteSpace(searchText))
            {
                ...
                {
                    Field = nameof(LoanProductMasterView.Name),
                    Operator = "contains",
                    value = searchText
                };

                WhereFilter guarantorFilter = new WhereFilter
                {
                    Field = nameof(LoanProductMasterView.IsGuarantorRequired),
                    Operator = "equal",
                    value = true
                };
                WhereFilter tenureFilter = new WhereFilter
                {
                    Field = nameof(LoanProductMasterView.TenureUnit),
                    Operator = "contains",
                    value = searchText
                };
                ...
                switch (filter)
                {
                    case "approved":
                        ...
                        QueryGrid = new Query().Where(
                            statusFilter.And(NameFilter.Or(guarantorFilter).Or(tenureFilter).Or(statusFilter)));
                        ActivateTab();
                        Approved = Active;
                        break;
                    case "rejected":
        ...
        private async Task DeactivateRecored(LoanProductMasterView row)
        {
        ...
        private async Task ConfirmDeactivateRecored()
        {
            try
            {
                ...
                {
                    UpdateLoanProductStatusCommand command = new UpdateLoanProductStatusCommand()
                    {
                        Id = LoanProductId,
                        Status = ProductStatus.DEACTIVATED
                    };
                    var rsp = await DataService.ProcessRequest<UpdateLoanProductStatusCommand, LoanProductViewModel>(
                        nameof(LoanProduct), "updateStatus", command);
        ...
        public async Task OnViewRow(LoanProductMasterView row)
        {
            _navigationManager.NavigateTo($"/LoanProductSetup/view/{row.Id}");
        }

        public async Task OnEditRow(LoanProductMasterView row)
        {
            _navigationManager.NavigateTo($"/LoanProductSetup/edit/{row.Id}");
        }
        ...
        private async Task onPublishToAllUser(LoanProductMasterView row)
        {
            command = new PublishLoanProductCommand();
            command.ProductId = row.Id;
            command.PublicationType = PublicationType.ALL;
            command.PublishedByUserId = CurrentUser.FindFirstValue(ClaimTypes.Sid);
            var rsp = await DataService.ProcessRequest<PublishLoanProductCommand, CommandResult<LoanProductViewModel>>(
                nameof(LoanProduct), "publish", command);
            Logger.LogInformation($"error->{JsonSerializer.Serialize(command)}");
            ...
            else
            {
                ...
                await _auditLogService.LogAudit("Publish Loan product to all user Creation.", "ID", "Security", payload,
                    CurrentUser);
        ...
        private async Task onPublishToDepartment(string Id)
        {
            try
            {
                ...
                //productPublicationCommand.ProductType = nameof(ProductType.Loan);
                //var CheckIfExistMembersToPublishToCommand = new CheckIfExistDepartmentsToPublishToCommand { ProductId = Id };
                //var rsp = await DataService.ProcessRequest<CheckIfExistDepartmentsToPublishToCommand, CommandResult<DepartmentsToPublishToViewModel>>("ProductPublications", "checkDepartmentByProductId", CheckIfExistMembersToPublishToCommand);
                //Logger.LogInformation($"error->{JsonSerializer.Serialize(rsp?.Error?.Content)}");
                ...
                //else
                //{
                //    var resp = JsonSerializer.Deserialize<DepartmentsToPublishToViewModel>(rsp.Content.Response.ToJson());
                //    departmentList = resp.DepartmentsToPublishTo;
                //}
        ...
        public async Task OnSaveDepartmentClose()
        {
            try
            {
                ...
                if (checkIsSuccessfull)
                {
                    ...
                    {
                        await _auditLogService.LogAudit("Publish Loan product to specific department Creation.", "ID",
                            "Security", payload, CurrentUser);
        ...
        private async Task onPublishToSpecificUser(string Id)
        {
            try
            {
                ...
                command.ProductId = Id;
                //productPublicationCommand.ProductType = nameof(ProductType.Loan);
                //var CheckIfExistMembersToPublishToCommand = new CheckIfExistMembersToPublishToCommand { ProductId = Id };
                //var rsp = await DataService.ProcessRequest<CheckIfExistMembersToPublishToCommand, CommandResult<MembersToPublishToViewModel>>("ProductPublications", "checkMembersByProductId", CheckIfExistMembersToPublishToCommand);
                //Logger.LogInformation($"payload->{JsonSerializer.Serialize(productPublicationCommand)}");
                //Logger.LogInformation($"error->{JsonSerializer.Serialize(rsp?.Error?.Content)}");
                ...
                //else
                //{
                //    var resp = JsonSerializer.Deserialize<MembersToPublishToViewModel>(rsp.Content.Response.ToJson());
                //    userList = resp.MembersToPublishTo;
                //}
